...
const IMGS = [
    'resources/jaen.png', 'resources/cordoba.png', 'resources/granada.png', 'resources/almeria.png', 'resources/malaga.png', 'resources/sevilla.png', 'resources/huelva.png', 'resources/cadiz.png', 'resources/andalucia.png'
];
...
const main = function() {
    ...
    form.addEventListener('submit', (e) => {
        e.preventDefault();
        let input = form.querySelector('input[name="pueblo"]');
        let input_ready = removeDiacriticChars(input.value.toLowerCase());
        searchingTown(input_ready);
    });
}

function createCheckboxes() {
    let div_checkboxes = document.getElementById('provincias');
    for (const i of Object.keys(PROVINCES)) {
        div_checkboxes.innerHTML += `<input type="checkbox" id="provincia_${i}" value="${i}"/><label for="provincia_${i}"> ${i}</label>`;
    }
}

function searchingTown(town) { // Estas buscando ciudad. Se raya cuando pones las capitales porque busca en todos los nombres completos
    let search = { ...PROVINCES };
    for (let province in search) {
        search[province].forEach((e, index) => {
            let pr_search = removeDiacriticChars(e.toLowerCase());  // ¿Por qué quieres pasarle los pueblos sin mayusculas y sin tildes, cuando el array original si los tienen?
            if (e.includes(town)) { // Si quieres hacerlo así, tienes que pasar también todos los pueblos del array a minisculas y sin tildes, si lo haces con pr_search no busca nada
console.log(index); // Me devuelve un 10 para Bailén? estas pasando el numero de poblacion de cada provincia a marcar y enseñar imagen
console.log(province) // Necesitas el nombre de la provincia, o es que las tienes bien catalogadas?
...
function markProvince(index){
    let chboxes = document.querySelectorAll('input[type="checkbox"]'); // Estas buscando por el indice, pero estas segura que es lo que buscas?
    //los dejo todos unchecked
    for(chbox of chboxes){
        chbox.checked = false; 

        if(chbox.value == Object.keys(PROVINCES)[index]){
            chbox.checked = true;
        }
    }
}

function showImage(index = 8){
    let img = document.querySelector('img');
    let route = IMGS[index];
    img.setAttribute('src', route);
}

function removeDiacriticChars(text) {
